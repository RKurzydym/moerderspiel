...
class WordGenerator(object):
    ...
    and it will derive its matrix of transition probabilities from any string it
    ...
    poss: Dict[str, Dict[str, int]] = {
        'a': {'a': 37, 'c': 2628, 'b': 1777, 'e': 213, 'd': 1848, 
        ...
        for line in fileinput.input(filename):
          for word in line.lower().strip().split(' '):
            if( re.match('^[a-z]+$', word) != None ):
              ...
              oc = chr(96)
              for c in list(word):
                if oc not in self.poss:
                    self.poss[oc] = {chr(i): 0 for i in range(97, 123)}  # Initialize with a-z
                if c not in self.poss[oc]:
                    self.poss[oc][c] = 0
                self.poss[oc][c] += 1
                oc = c
        fileinput.close()
